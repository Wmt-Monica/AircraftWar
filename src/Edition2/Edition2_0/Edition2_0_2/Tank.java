...
public class Tank extends JFrame {
    ...
    public Tank(int x, int y,boolean good,Wall wall1,Wall wall2){
        this.x = x;
        this.y = y;
        this.good = good;
        this.wall1 = wall1;
    ...
    public boolean crossScreen(int nowX,int nowY){
        if (nowX < 10 || nowX > new TankClient().getScreenWidth()-40 || nowY < 30 || nowY > new TankClient().getScreenHeight()-40
                || (nowX+TANK_SIZE >= wall1.getX() && nowX <= wall1.getX()+wall1.getWeight() && nowY+TANK_SIZE >= wall1.getY() && nowY <= wall1.getY()+wall1.getHeight())
        || (nowX+TANK_SIZE >= wall2.getX() && nowX <= wall2.getX()+wall2.getWeight() && nowY+TANK_SIZE >= wall2.getY() && nowY <= wall2.getY()+wall2.getHeight())){
            return false;
        }
        return true;
    }
