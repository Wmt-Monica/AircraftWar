...
public class EnemyTank implements Runnable {
    ...
    public void creatEnemyTank(){
        ...
        if (enemyTankNum < 10){  //如果游戏界面上敌方坦克的个数少于10个就创建新的敌方坦克
            int x = new Random().nextInt(SCREEN_WIDTH - 60)+10;
            int y = new Random().nextInt(SCREEN_HEIGHT - 60)+30;
            //随机获取敌方坦克的位置
            while (!(Math.abs(x - myTank.getX()) <= 200) && !(Math.abs(y - myTank.getY()) <= 200)){
                 x = new Random().nextInt(SCREEN_WIDTH - 60)+10;
                 y = new Random().nextInt(SCREEN_HEIGHT - 60)+30;
            }
